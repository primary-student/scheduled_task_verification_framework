...
def load_tasks_and_start():
    ...
    new_task_on_every_month_1st = scheduler_task_manager_obj.is_cronTask(name = "month_1st",task_scheduled_run_at = now_is_month_1st)(task_on_every_month_1st)


    scheduler.add_job(new_task_on_every_monday, 'cron',day_of_week="mon", hour=1, minute=1)
    scheduler.add_job(new_task_on_every_month_1st, 'cron', day=1, hour=1, minute=1)
    scheduler.start()
